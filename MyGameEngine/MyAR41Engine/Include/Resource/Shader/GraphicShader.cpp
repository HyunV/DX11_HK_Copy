...
{
	D3D11_INPUT_ELEMENT_DESC Desc = {}; //인풋 레이아웃 구조체 선언, 일단 초기화
	//각 구조체 성분이 어떤 용도인지 알려주기 위한녀석
...
bool CGraphicShader::LoadVertexShader(const char* EntryName, const TCHAR* FileName, const std::string& PathName)
{
	...
	const PathInfo* Info = CPathManager::GetInst()->FindPath(PathName); //경로 찾아온다.

	if (Info)
		lstrcpy(FullPath, Info->Path); //경로 복사

	lstrcat(FullPath, FileName); //경로+파일이름 결합
	...
	//위에서 컴파일한 코드를 이용해 VertexShader 객체를 만들어낸다.
